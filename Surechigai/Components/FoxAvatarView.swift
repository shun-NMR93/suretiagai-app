...
struct FoxAvatarView: View {
    ...
    private func foxEar(side: CGFloat) -> some View {
        ZStack {
            earShape(furColor: fur.main)
            earShape(furColor: fur.innerEar)
                .scaleEffect(0.6)
                .offset(y: size * 0.02)
        }
    ...
    @ViewBuilder
    private func earShape(furColor: Color) -> some View {
        switch avatar.earShape {
        case 0: // 三角
            FoxEarShape()
                .fill(furColor)
        ...
        case 1: // 丸み
            Capsule()
                .fill(furColor)
                .frame(width: size * 0.18, height: size * 0.24)
        case 2: // 垂れ耳
            Ellipse()
                .fill(furColor)
